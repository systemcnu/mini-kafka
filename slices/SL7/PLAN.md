...
1. **Short-circuit:** `if (RENDER_URL === "RENDER_URL_TBD") { showFallback(); return; }` — NEVER fetch (a bare placeholder resolves as a RELATIVE url against github.io, and fetch resolves on a 404 — the naive "resolved = live" predicate would redirect visitors to a broken path).
